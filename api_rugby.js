...
let query = "https://api.the-odds-api.com/v4/sports/rugbyleague/odds/?apiKey=" + KEY + "&regions=eu";
    ...
    .then(data => {
        ...
        data.forEach((liga, index) => {
            if (meczDivs[index]) {
                ...
                const remis = meczDivs[index].querySelector('.remis');
                const draw = meczDivs[index].querySelector('.kurs2');
                ...
                if (liga.bookmakers && liga.bookmakers.length > 0 &&
                    ...
                    const outcomes = liga.bookmakers[2].markets[0].outcomes;
                    ...
                    if (remis) {
                        remis.textContent = "Remis:";
                    }
                    if (draw && outcomes[2]) {
                        draw.textContent = outcomes[2].price;
                    }
